...
class RetryResult(CommitResult):
    """Result indicating commit should be retried."""
    ...
    def __init__(self, latest_snapshot, base_data_files, exception: Optional[Exception] = None):
        ...
        Args:
            latest_snapshot: The latest snapshot at the time of failure
            base_data_files: Base data files that were read
...
class FileStoreCommit:
    ...
    def _try_commit(self, commit_kind, commit_entries, commit_identifier):
        ...
        while True:
            ...
            # Check retry limits (matching Java logic exactly)
            elapsed_ms = int(time.time() * 1000) - start_time_ms
            if elapsed_ms > self.commit_timeout or retry_count >= self.commit_max_retries:
                # Show actual elapsed time in error message (more helpful than timeout value)
                error_msg = (
                    f"Commit failed after {elapsed_ms} millis with {retry_count} retries, "
    ...
    def _generate_overwrite_entries(self, latest_snapshot):
        ...
        entries = []
        # Get current files to delete (based on partition filter)
    ...
    def _commit_retry_wait(self, retry_count: int):
        ...
        logger.debug(
            f"Waiting {total_wait_ms}ms before retry (base: {retry_wait_ms}ms, jitter: {jitter_ms}ms)"
        )
    ...
    def _try_commit_once(self, retry_result: Optional[RetryResult], commit_kind: str,
        ...
        try:
            ...
            self._cleanup_preparation_failure(created_manifest_file, created_delta_manifest_list, created_base_manifest_list)
            raise RuntimeError(
                f"Exception occurs when preparing snapshot #{new_snapshot_id} by user {self.commit_user} "
                f"with identifier {commit_identifier} and kind {commit_kind}. Clean up."
            ) from e
        ...
        try:
            with self.snapshot_commit:
                ...
                if not success:
                    ...
                    logger.warning(
                        f"Atomic commit failed for snapshot #{new_snapshot_id} by user {self.commit_user} "
            ...
            logger.warning(f"Retry commit for exception: {e}", exc_info=True)
            return RetryResult(latest_snapshot, base_data_files, e)
        
        logger.info(
            f"Successfully commit snapshot {new_snapshot_id} to table {self.table.identifier.get_full_name()} "
            f"by user {self.commit_user} with identifier {commit_identifier} and kind {commit_kind}."
        )
        return SuccessResult()
    ...
    def _cleanup_preparation_failure(self, manifest_file: Optional[str], 
        ...
        try:
            ...
            if delta_manifest_list:
                try:
                    # Read manifest list and delete manifest files
    ...
    def _cleanup_commit_failure(self, base_manifest_list: str, delta_manifest_list: str):
        try:
            ...
            # Clean up delta manifest list (but not the manifest files, as they may be reused)
            if delta_manifest_list:
                delta_path = f"{manifest_path}/{delta_manifest_list}"
                self.table.file_io.delete_quietly(delta_path)
        except Exception as e:
