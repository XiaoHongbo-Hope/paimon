...
class TableRead:
    ...
    def to_ray(self, splits: List[Split], use_distributed_read: bool = True) -> "ray.data.dataset.Dataset":
        ...
        Args:
            ...
            use_distributed_read: If True, use distributed parallel reading via Datasource.
                                 If False, use the simple approach (loads all data first).
                                 Default is True for better performance with large datasets.
        ...
        if use_distributed_read:
            # Use distributed parallel reading via custom Datasource
            # This allows Ray to read multiple splits in parallel across the cluster
            try:
                ...
                # Fallback to simple approach if Datasource import fails
                return ray.data.from_arrow(self.to_arrow(splits))
        else:
            # Simple approach: load all data first, then convert to Ray Dataset
