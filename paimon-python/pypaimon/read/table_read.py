...
class TableRead:
    ...
    def to_blob_iterator(self, splits: List[Split]) -> Iterator:
        ...
        After read-path alignment with Java, to_iterator() returns rows whose
        BLOB fields are raw stored bytes (descriptor or inline) and lazy
        Blob access works via row.get_blob(pos).
        """
