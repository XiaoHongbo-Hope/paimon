...
class _IncrementalUpdateCommitter:
    ...
    def add_group(self, commit_messages, _num_updated, _row_ids) -> None:
        if self._deferred_error is not None:
            # Drain workers without retaining later uncommitted files.
            self._abort_messages(commit_messages)
            return
    ...
    def _commit_pending(self) -> None:
        ...
        if self._table_commit is None:
            try:
                ...
                self._table_commit.enable_bounded_row_id_conflict_state()
            except Exception as error:
    ...
    def abort_pending(self) -> None:
        ...
        self._pending_messages = []
        self._pending_groups = 0
        self._abort_messages(pending_messages)

    def _abort_messages(self, messages) -> None:
        if not messages:
            return
        ...
        try:
            self._table_commit.abort(messages)
...
def _commit_update_messages(table, commit_messages) -> None:
    ...
    try:
        ...
        if (not commit_started
                or not isinstance(e, CommitOutcomeUnknownError)):
