...
from pypaimon.read.scanner.data_evolution_manifest_filter import _evolution_stats
...
class DataEvolutionTest(unittest.TestCase):
    ...
    def test_with_filter_and_projection(self):
        ...
        pb = rb_full.new_predicate_builder()
        predicate_compound = pb.and_predicates([
            pb.greater_than("c", 150),
            pb.is_not_null("c"),
        ])
        rb4_filtered = table.new_read_builder().with_projection(["id", "c"]).with_filter(predicate_compound)
        ...
        expected_output_arity = len(table_read.output_type)
        try:
    ...
    def test_null_predicate_arrow_vs_row_by_row_semantics(self):
        ...
        self.assertTrue(pred_is_null.has_null_check())
    ...
    def test_evolution_stats_row_count(self):
        ...
        b_field = DataField(1, "b", AtomicType("INT"))
        c_field = DataField(2, "c", AtomicType("INT"))
        schema_0 = [id_field, b_field]
        schema_1 = [c_field]
        table_fields = [id_field, b_field, c_field]
        ...
        def _make_file_meta(
            file_name, row_count, schema_id, write_cols, value_stats,
            first_row_id, max_sequence_number,
        ):
            return DataFileMeta(
                file_name=file_name,
                file_size=1024,
                ...
                level=0,
                extra_files=[],
                value_stats_cols=write_cols,
                write_cols=write_cols,
                first_row_id=first_row_id,
            )

        file_a = _make_file_meta(
            "data-a.parquet", 2, 0, ["id", "b"],
            SimpleStats(
                GenericRow([1, 10], schema_0),
                GenericRow([2, 21], schema_0),
                [0, 0],
            ),
            0, 3,
        )
        file_b = _make_file_meta(
            "data-b.parquet", 3, 1, ["c"],
            SimpleStats(
                GenericRow([200], schema_1),
                GenericRow([200], schema_1),
                [2],
            ),
            0, 2,
        ...
        entries = [
            ManifestEntry(0, GenericRow([], []), 0, -1, file_a),
            ManifestEntry(0, GenericRow([], []), 0, -1, file_b),
        ]

        _, row_count = _evolution_stats(
