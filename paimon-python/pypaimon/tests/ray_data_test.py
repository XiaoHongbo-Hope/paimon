...
class RayDataTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Try to import ray, skip tests if not available
        try:
            import ray
            cls.ray_available = True
            # Initialize Ray if not already initialized
            if not ray.is_initialized():
                ray.init(ignore_reinit_error=True, num_cpus=2)
        except ImportError:
            cls.ray_available = False
            print("Ray is not available, skipping Ray Data tests")

    @classmethod
    def tearDownClass(cls):
        """Clean up test environment."""
        try:
            if cls.ray_available:
                import ray
    ...
    def setUp(self):
        """Skip test if Ray is not available."""
    ...
    def test_basic_ray_data_read(self):
        ...
        self.assertEqual(list(df['id']), [1, 2, 3, 4, 5], "ID column should match")
        self.assertEqual(list(df['name']), ['Alice', 'Bob', 'Charlie', 'David', 'Eve'], "Name column should match")
    ...
    def test_ray_data_map_operation(self):
        ...
        self.assertEqual(list(df['value']), [20, 40, 60, 80, 100], "Values should be doubled")
